...
class DScribeApp {
    ...
    /**
     * Handle audio separator dialog
     */
    showAudioSeparatorDialog() {
        const modal = document.getElementById('audio-separator-modal');
        modal.style.display = 'flex';
        
        const fileInput = document.getElementById('audio-file-input');
        const uploadBtn = document.getElementById('btn-upload-audio');
        const closeBtn = document.getElementById('btn-close-separator');
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            // Initialize separator if not done
            if (!window.audioSeparator) {
                window.audioSeparator = new AudioSeparator();
                await window.audioSeparator.initialize();
            }
            
            // Load audio file
            const result = await window.audioSeparator.loadAudioFile(file);
            if (!result.success) {
                alert(`Fehler beim Laden der Datei: ${result.error}`);
                return;
            }
            
            // Show progress
            document.getElementById('separator-progress').style.display = 'block';
            window.audioSeparator.onProgress = (progress) => {
                document.getElementById('separator-progress-bar').style.width = (progress * 100) + '%';
                document.getElementById('separator-status').textContent = `${Math.round(progress * 100)}%`;
            };
            
            // Perform separation
            const separateResult = await window.audioSeparator.separateTracks();
            
            if (separateResult.success) {
                document.getElementById('separator-results').style.display = 'block';
                document.getElementById('separator-progress').style.display = 'none';
            } else {
                alert(`Fehler bei der Trennung: ${separateResult.error}`);
            }
        });
    ...
    showOMRDialog() {
        ...
        const closeBtn = document.getElementById('btn-close-omr');
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            // Initialize OMR if not done
            if (!window.omrEngine) {
                window.omrEngine = new OMREngine();
                await window.omrEngine.initialize();
            }
            
            // Show progress
            document.getElementById('omr-progress').style.display = 'block';
            window.omrEngine.onProgress = (progress) => {
                document.getElementById('omr-progress-bar').style.width = (progress * 100) + '%';
                document.getElementById('omr-status').textContent = `${Math.round(progress * 100)}% - ${this.getOMRProgressText(progress)}`;
            };
            
            // Process PDF
            const omrResult = await window.omrEngine.processPDF(file);
            
            if (omrResult.success) {
                const confidence = omrResult.musicData.confidence;
                document.getElementById('omr-staves-count').textContent = window.omrEngine.stavesFound.length;
                document.getElementById('omr-notes-count').textContent = window.omrEngine.detectedNotes.length;
                document.getElementById('omr-measures-count').textContent = window.omrEngine.detectedMeasures.length;
                document.getElementById('omr-confidence-score').textContent = Math.round(confidence.overall * 100) + '%';
                
                document.getElementById('omr-results').style.display = 'block';
                document.getElementById('omr-progress').style.display = 'none';
            } else {
                alert(`Fehler bei der PDF-Verarbeitung: ${omrResult.error}`);
            }
        });
    ...
    /**
     * Get OMR progress text
     */
    getOMRProgressText(progress) {
        if (progress < 0.25) return 'PDF konvertieren...';
        if (progress < 0.5) return 'Notensysteme erkennen...';
        if (progress < 0.75) return 'Noten detektieren...';
        return 'Takte gruppieren...';
    }
    ...
    showAudioExportDialog() {
        ...
        const closeBtn = document.getElementById('btn-close-audio-export');
        ...
        exportBtn.addEventListener('click', async () => {
            ...
            this.setStatus('Exportiere als Audio...');
            
            // Save using file dialog
            const filePath = await window.electron.file.saveDialog({
                title: 'Audio exportieren als',
                defaultPath: `${this.currentProject.title}.${format}`,
                filters: [
                    { name: 'Audio Dateien', extensions: [format] },
                    { name: 'Alle Dateien', extensions: ['*'] }
                ]
            });
            ...
            if (filePath) {
                try {
                    await window.electron.audio.exportAudio(this.currentProject, filePath, format, quality);
                    alert(`Audio erfolgreich exportiert nach:\n${filePath}`);
                    this.setStatus('Audio exportiert');
                    modal.style.display = 'none';
                } catch (error) {
     ...
     */
    handleImportAudio() {
        this.showAudioSeparatorDialog();
    }

    /**
     * Handle import PDF OMR
     */
    handleImportPDF() {
        this.showOMRDialog();
    }
     ...
     */
    handleExportAudio() {
        this.showAudioExportDialog();
    }

    /**
     * Setup audio separator event listeners
     */
    setupAudioSeparatorListeners() {
        // Play track button
        document.querySelectorAll('.btn-play-track').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const track = e.target.dataset.track;
                if (window.audioSeparator) {
                    window.audioSeparator.playTrack(track);
                }
            });
        });
        
        // Export track button
        document.querySelectorAll('.btn-export-track').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const track = e.target.dataset.track;
                if (window.audioSeparator) {
                    const wavBlob = await window.audioSeparator.exportTrackAsWAV(track);
        ...
        // Import separated tracks
        document.getElementById('btn-import-separated')?.addEventListener('click', () => {
            alert('Importierte Spuren werden zur Weiterverarbeitung vorbereitet...');
            // Import tracks into separate instruments
        });
    }

    /**
     * Setup OMR event listeners
     */
    setupOMRListeners() {
        document.getElementById('btn-import-omr')?.addEventListener('click', async () => {
            if (!window.omrEngine) return;
            
            const musicData = window.omrEngine.getMusicData();
            this.currentProject = musicData;
            this.notationEngine.loadProject(musicData);
            this.markProjectDirty();
            
            alert('PDF-Noten erfolgreich importiert!');
            document.getElementById('omr-modal').style.display = 'none';
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    app.setupAudioSeparatorListeners();
    app.setupOMRListeners();
});
